...
public class RenderParticles implements IComponent, IRender, IScript
{
    ...
    {
            throw new Error("[ERROR] setAngle method is not used in RenderParticles class !");
    }
    ...
    public void setPosition(int emitterIndex, Vector2f newPos)
    {
        if(newPos != null)
        {
            ...
            {
                ConfigurableEmitter pe = this.emitters.get(emitterIndex);
                if(pe != null)
                {
                    pe.setPosition(newPos.x, newPos.y, false);
                }
            }
            else
            {
