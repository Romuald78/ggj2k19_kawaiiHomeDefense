...
import fr.rphstudio.ecs.core.utils.ControllerAL.Axis;
        ...
        for(int entIdx=0;entIdx<entList.size();entIdx++)
        {
            ...
            for(int cmpIdx=0;cmpIdx<cmpList.size();cmpIdx++)
            {
                ...
                if(cmpList.get(cmpIdx).getClass() == AnalogPadHandler.class)
                {
                    ...
                    // Update pad position
                    int idAxisX = ControllerAL.getAxisNumber(ctrlID, Axis.AXIS_X);
                    int idAxisY = ControllerAL.getAxisNumber(ctrlID, Axis.AXIS_Y);
                    pad.updatePosition( in.getAxisValue(ctrlID, idAxisX), in.getAxisValue(ctrlID, idAxisY) );
                    // Update rotation
                    int idRotX  = ControllerAL.getAxisNumber(ctrlID, Axis.ROTATE_X);
                    int idRotY  = ControllerAL.getAxisNumber(ctrlID, Axis.ROTATE_Y);
                    int idRotZ1 = ControllerAL.getAxisNumber(ctrlID, Axis.ROTATE_Z1);
                    int idRotZ2 = ControllerAL.getAxisNumber(ctrlID, Axis.ROTATE_Z2);
                    // First init values of axis before trying to get the value
                    float vX = 0.0f;
                    float vY = 0.0f;
                    float vZ = 0.0f;
                    // Get the X value
                    if( idRotX >= 0 )
                    {
                        vX = in.getAxisValue(ctrlID, idRotX);
                    }
                    // Get the Y value
                    if( idRotY >= 0 )
                    {
                        vY = in.getAxisValue(ctrlID, idRotY);
                    }
                    // Get the two (or at least one) values for Z : proceed Zleft - Zright to obtain the final Z value
                    if( idRotZ1 >= 0 )
                    {
                        vZ += in.getAxisValue(ctrlID, idRotZ1);
                    }
                    if( idRotZ2 >= 0 )
                    {
                        vZ -= in.getAxisValue(ctrlID, idRotZ2);
                    }
                    // apply a ratio depending on the controller (so the output is always between -1 and +1
                    vZ *= ControllerAL.getZAxisRatio(ctrlID);
                    // update rotation value
                    pad.updateRotation(vX, vY, vZ);
                }                
            }
        }
        // Measure execution time
